...
/// </summary>
public partial class AcmeCertificateSelector(
    ICertificateStore certificateStore,
    ILogger<AcmeCertificateSelector> logger)
{
    /// <summary>
    ...
    static partial void LogNoSniHostnameProvidedInClienthello(ILogger<AcmeCertificateSelector> logger);

    [LoggerMessage(LogLevel.Debug, "Found certificate for host: {host}")]
    static partial void LogFoundCertificateForHostHost(ILogger<AcmeCertificateSelector> logger, string host);

    [LoggerMessage(LogLevel.Debug, "Found wildcard certificate for host: {host} ({wildcard})")]
    static partial void LogFoundWildcardCertificateForHostWildcard(ILogger<AcmeCertificateSelector> logger, string host,
        string wildcard);

    [LoggerMessage(LogLevel.Warning, "No certificate found for host: {host}")]
    static partial void LogNoCertificateFoundForHost(ILogger<AcmeCertificateSelector> logger, string host);
}